...
- The export form intentionally requires the owner to enter terrain width excluding the fixed exterior frame, base thickness, and vertical exaggeration. No unapproved terrain or base defaults are silently applied.
...
- Raised routes use an original height-field offset over the same terrain mesh, not a separate shell. GPX segments are projected into the shared local model coordinates, clipped to the regular hexagon, and retain their gaps. Width and rise are required explicit inputs; the server rejects a width too narrow for the current DEM grid. The owner has excluded recessed routes from this application.
- Raised-route edges use a linear transition no wider than one terrain-grid interval and contained within the owner-selected route width. This removes voxel sidewalls while preserving the selected route footprint and height; it is not a global terrain blur.
...
- The browser workflow is presented as four gated, wizard-style stages: Upload GPX, Map & terrain area, Fine-tune settings, and 3D model viewer. This is visually tabbed but uses ordinary buttons and focused stage headings rather than ARIA tabs because each panel is a substantial task with prerequisites. Earlier ready stages remain revisitable, generation moves to the viewer, and changing generation settings still marks the exact-STL preview stale.
- The repository was paused on 2026-09-14 after the four-stage workflow was completed and reviewed. The next implementation work is deliberately validation rather than new features: Docker startup, target-slicer inspection, and physical-print testing. A two-tone raised-route workflow was discussed but is not in scope: standard STL has no portable material assignment, and any separate-material or 3MF export must wait for that validation evidence and a deliberate geometry/export design.
- The deployment image uses the official `node:22-bookworm-slim` image in a multi-stage Dockerfile. The build stage creates the Vite client and bundled server; the runtime stage contains only production dependencies and the generated artifacts, runs as the unprivileged `node` user, and serves the client and `/api` from one port. The elevation key is injected only at runtime through `.env`/container environment; `.dockerignore` excludes environment files and raw sample routes. `/api/health` is the image health check. The current development machine has no Docker installation, so a real container startup is not yet claimed. [Official Node image guidance](https://github.com/nodejs/docker-node) · [Dockerfile reference](https://docs.docker.com/reference/dockerfile) · [Compose environment guidance](https://docs.docker.com/compose/how-tos/environment-variables/set-environment-variables/)
