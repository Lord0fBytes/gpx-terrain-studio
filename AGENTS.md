...
- Keep the geographic selection size separate from printed size. Use a single configured polygon shape and geographic footprint in v1; allow repositioning, with rotation only if useful. Changing printed width scales the same selection uniformly and derives the other planar dimension from its aspect ratio.
- The exact polygon, geographic footprint, and initial print dimensions are not decided. Record initial values explicitly as provisional before implementation; do not silently treat earlier illustrative dimensions as requirements.
