...
The prototype validates GPX files, preserves their segments on a Leaflet map, derives a regular flat-top hexagonal selection from route bounds plus the provisional 20% context rule, fetches a bounded OpenTopography DEM, and generates a watertight STL in millimeters. The entered width controls the terrain footprint; every export adds a 6 mm frame beyond each side (100 mm terrain becomes 112 mm overall), with its top 5 mm above the configured base thickness. An integrated raised route is optional; recessed routes are out of scope. The browser previews the exact generated STL bytes in Three.js and reuses those bytes for the separate download action. A single-container configuration serves both the built browser app and API. Slicer validation and physical-print validation remain unfinished.
